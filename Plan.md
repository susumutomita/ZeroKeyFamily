...
### API 端末セッション認証層の設計（Issue 13・設計フェーズ） - 2026-06-27

#### 目的

Issue 13（API 端末セッション認証層）は全書き込みエンドポイント・既存テスト・フロントエンドへ波及する invasive な変更のため、作業順序（ドキュメント → リファクタリング → 機能追加）に従い、実装に先立って設計判断を ADR で固定する。Issue: https://github.com/susumutomita/ZeroKeyFamily/issues/13 とする。

#### 制約

- 設計判断は ADR で記録する（AGENTS.md）。
- 秘密鍵を端末外に出さない原則と、依頼内容への端末署名（content signature）の独立性を維持する。
- ドキュメント規則に従い textlint を通す。

#### タスク

1. ADR-0004（端末セッション認証層の設計・Proposed）を作成し、プロトコル・拒否マトリクス・段階的ロールアウトを定義する。
2. 設計をレビュー・基盤 PR（脅威モデル / 監査ログ）のマージ後に、実装フェーズ（ハンドシェイクエンドポイント → 書き込みへの必須化 → フロントエンド）へ進む。

#### 検証手順

- textlint と architecture-harness が Green。

#### 進捗ログ

- 2026-06-27: ブランチ `claude/adr-session-auth`（main 起点）で着手。ADR-0004 を作成。実装は invasive かつ基盤（PR #16 / #17）が未マージのため、設計を先行させ実装フェーズは基盤マージ後に分離する。
